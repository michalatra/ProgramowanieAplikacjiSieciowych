...
class POP3Client:
    ...
    def get_messages_max_size(self) -> OperationResult:
        ...
        if header[0] != POP3ResponseState.OK.value:
            print('An error occurred while getting messages')
            return OperationResult.FAIL
        
        if len(header) != 4:
            print('An error occurred while parsing messages')
            return OperationResult.FAIL
        ...
        try:
            ...
            for message in data:
                ...
                if self.get_message_content(message[0]) != OperationResult.SUCCESS:
                    print('An error occurred while getting message {} content'.format(message[0]))
                    return OperationResult.FAIL
                
                messages_contents.append((message[0], self.message_content))

            print(messages_contents)

            for (message_id, message_content) in messages_contents:
                print('-------------------------  Content of Message {}  -------------------------'.format(message_id))
    ...
    def get_message_content(self, message_id: str) -> OperationResult:
        if not self.is_logged_in:
            print('Not logged in')
            return OperationResult.FAIL

        self.socket.sendall((POP3RequestKeyword.RETR.value + ' ' + message_id + CRLF).encode())
        data = self.socket.recv(1024)
        print("Recieved: ", data.decode())
        ...
        if header[0] != POP3ResponseState.OK.value:
            print('An error occurred while getting message content')
            return OperationResult.FAIL
        
        self.message_content = CRLF.join(data[1:-1])
        return OperationResult.SUCCESS
...
if __name__ == '__main__':
    ...
    while client.get_messages_max_size() != OperationResult.SUCCESS:
        user_response = input("An error occurred while retrieving contents. Do you want to try again? (y/n): ")
        if user_response.lower() != 'y':
            exit(1)  
