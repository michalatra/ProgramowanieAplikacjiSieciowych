...
class POP3Client:
    ...
    def get_messages_max_size(self) -> OperationResult:
        ...
        if header[0] != POP3ResponseState.OK.value:
            print('An error occurred while getting messages max size')
            return OperationResult.FAIL
        
        if len(header) != 4:
            print('An error occurred while parsing messages max size')
            return OperationResult.FAIL
        ...
        try:
            ...
            for message in data:
                ...
                if size > max_size:
                    max_size = size
                    max_size_message_id = message[0]

            print('Message {} has a size of {} bytes'.format(message[0], message[1]))
            
            if self.get_message_content(max_size_message_id) != OperationResult.SUCCESS:
                print('An error occurred while getting message content')
                return OperationResult.FAIL


            print('---------------------------------  Content of Message with Max Size  ---------------------------------')
